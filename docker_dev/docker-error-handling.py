...
def createNewDockerfile(err):
    container, commands, err_msg = getImageAndCommands()

    print("Err is {}".format(err))
        
    # copy the Dockerfile to a readable text version
    scp_cmd = 'scp Dockerfile Dockerfile.txt'
    try:
        subprocess.check_output(scp_cmd, shell=True)
    except subprocess.CalledProcessError as cmdexep:
        raise cmdexep

    # open readable version
    with open('Dockerfile.txt') as docker_file:
        lines = []
        for line in docker_file:
            lines.append(line)
        print(lines[:6])
        err_part = err.split('&&', 1)[0]
        loc = get_close_matches(err_part, lines)
        print(loc)

def getImageAndCommands():
